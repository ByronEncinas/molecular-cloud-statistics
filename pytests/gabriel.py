...
def uniform_in_3d(no, rloc=1.0, threshold=100): # modify
    uniform_in_3d.c = 0
    def xyz_gen(size):
    ...
    while len(valid_vectors) < no:
        ...
        valid_mask = Density[nearest_indices] * gr_cm3_to_nuclei_cm3 > threshold
...
if False: # Note to self, for small rloc < 1 this code doesnt take long
    rho = uniform_in_3d(100_000, rloc=0.25, threshold=threshold)
...
with open(os.path.join(new_folder, f'PARAMETERS'), 'w') as file:
    ...
    file.write(f"Density Threshold : {threshold}\n")
...
# starting positions 
print(max_cycles)
x_init = uniform_in_3d(max_cycles, rloc, threshold)
null_vector = np.array([[0.0, 0.0, 0.0]])  # shape (1, 3)
x_input = np.vstack([x_init, null_vector]) 

print("No. of starting pos", x_input.shape)

radius_vector, numb_densities, average_column = line_of_sight(x_input, directions, )
