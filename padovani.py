...
if True:
    ...
    mu_ism = np.array([1.0])# np.logspace(-2, 0, num=50) #np.linspace(0, 1, mus) # 
...
for line in range(lines):
    ...
    if False:
        ...
        #plt.xscale('log')
        plt.yscale('log')
        plt.savefig(f'./images/i_rate/Model{model}_forward_column.png')  # Save as PNG with high resolution
        plt.close()
        ...
        #plt.xscale('log')
        plt.yscale('log')
        plt.savefig(f'./images/i_rate/Model{model}_backward_column.png')  # Save as PNG with high resolution
        plt.close()
        ...
        plt.xscale('log')
        plt.savefig(f'./images/i_rate/Model{model}_forward_jacobian.png')  # Save as PNG with high resolution
        plt.close()
        ...
        plt.ylabel(r"$\frac{\partial \mu_i}{\partial \mu}$")
        plt.xscale('log')
        plt.savefig(f'./images/i_rate/Model{model}_backward_jacobian.png')  # Save as PNG with high resolution
        plt.close()

    max_arg = np.argmax(magnetic_field)

    Nmir_fwd = mirrored_column_density(radius_vector[:max_arg+1,:], magnetic_field[:max_arg+1], numb_density[:max_arg+1], Npmu[:max_arg+1,:], 'mir_fwd')
    ...
    zeta_mir_fwd,  zeta_mui_mir_fwd  = ionization_rate(Nmir_fwd, mu_local_fwd[:max_arg+1,:], dmu_fwd[:max_arg+1,:], 'mir_fwd')
    zeta_mir_bwd, zeta_mui_mir_bwd = ionization_rate(Nmir_bwd, mu_local_bwd[max_arg:,:], dmu_bwd[max_arg:,:], 'mir_bwd')

    #zeta_mir_fwd, zeta_mui_mir_fwd  = ionization_rate(Nmir_fwd, mu_local_fwd, dmu_fwd, 'mir_fwd')
    #zeta_mir_bwd, zeta_mui_mir_bwd   = ionization_rate(Nmir_bwd, mu_local_bwd, dmu_bwd, 'mir_bwd')
...
#zeta_reduced = zeta_at_x[mask]
#nmir_reduced = nmir_at_x[mask]
...
ax.scatter(nmir_at_x, zeta_at_x, s=5, marker='x', color="red", label=fr"$\zeta$", alpha=0.2)
ax.plot(Neff, 10**log_H(Neff), label='Model H')
ax.plot(Neff, 10**log_L(Neff), label='Model L')
