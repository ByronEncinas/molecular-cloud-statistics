...
for fileno, filename in enumerate(file_list):
    ...
    region_radius = 0.1
    """
    c = (xc**2 + yc**2 + zc**2 < region_radius**2)
    c_Pos = Pos[c,:] - center0
    fig, ax = plt.subplots()

    #d = sorted(np.linalg.norm(c_Pos, axis=1))
    #ax.plot(d)
    ax.scatter(xc[c], yc[c], s=5)
    plt.savefig('./a.png')
    break
    """
    for each in range(how_many):            
        if each == 0:
            ...
            c = ((xc - CloudCord[0])**2 + (yc - CloudCord[1])**2 + (zc - CloudCord[2])**2 < region_radius**2)
    ...
    for each, cloud in enumerate(centers):
        ...
        sp.annotate_text(
            [cloud[0], cloud[1], cloud[2]],  # Slightly offset to avoid overlap
        ...
        sp.annotate_sphere(
            [cloud[0], cloud[1], cloud[2]],  # Center of the sphere
            radius=(10, "pc"),  # Radius of the sphere (in physical units, e.g., pc)
            circle_args={"color": "black", "linewidth": 2}  # Styling for the sphere
        )
    ...
    for each, cloud in enumerate(centers):
        ...
        sp.annotate_sphere(
            [cloud[0], cloud[1], cloud[2]],  # Center of the sphere
            radius=(0.2, "pc"),  # Radius of the sphere (in physical units, e.g., pc)
            circle_args={"color": "black", "linewidth": 2}  # Styling for the sphere
        )
    ...
    sp.save(f"./clouds/{input_case}/{snap}/all.png")
    data.close()
